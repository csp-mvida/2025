...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBack }) => {
  ...
  const getInvoiceUrls = (request: CSPRequest): string[] => {
    if (!request.invoiceUrl) return [];
    if (request.invoiceUrl === 'Pendente via WhatsApp') return [];
    try {
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
             {(['all', 'pending', 'approved', 'paid', 'rejected'] as const).map(status => (
               <button key={status} onClick={() => setStatusFilter(status)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors border ${statusFilter === status ? 'bg-primary text-white border-primary shadow-md shadow-primary/20' : 'bg-white/80 backdrop-blur-sm border-slate-200 text-slate-600 hover:bg-slate-50'}`}>{status === 'all' ? 'Todos' : STATUS_CONFIG[status].label}</button>
             ))}
             <button onClick={loadData} className="p-2.5 rounded-lg bg-white/80 backdrop-blur-sm border border-slate-200 text-slate-500 hover:text-primary ml-auto"><RefreshCw className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} /></button>
          </div>
        </div>
                      ...
                      <div className="space-y-2">
                        {getInvoiceUrls(selectedRequest).map((url, i) => (
                          <a key={i} href={url} target="_blank" rel="noreferrer" className="flex items-center gap-2 p-2 bg-slate-50 rounded border border-slate-200 text-xs text-primary font-bold hover:bg-primary/5 transition-all">
                            <Download className="w-3 h-3" /> Anexo {i + 1}
                          </a>
                        ))}
                      </div>
                    ) : (
                      <span className="text-accent text-sm italic flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> {selectedRequest.invoiceUrl || 'Nenhum anexo'}</span>
                    )}
                    {selectedRequest.boletoUrl && (
                      <a href={selectedRequest.boletoUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 p-2 bg-amber-50 rounded border border-amber-100 text-xs text-amber-600 font-bold hover:bg-amber-100 transition-all">
