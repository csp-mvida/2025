...
import {
  ...
  submitRequest, uploadInvoice, createDraftRequest
} from './services/api';
...
import {
  ...
  Home, Lock, Copy, Trash2, Info, Clock, X
...
function App() {
  ...
  const handleRemoveFile = (idx: number, type: 'invoice' | 'boleto' | 'transfer') => {
    ...
    else setTransferFiles(prev => prev.filter((_, i) => i !== idx));
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'invoice' | 'boleto' | 'transfer') => {
    ...
    const MAX_FILES = 10;
    const currentCount =
      type === 'invoice' ? invoiceFiles.length :
      type === 'boleto' ? boletoFiles.length :
    ...
    else setTransferFiles(prev => [...prev, ...files]);
  };
  ...
  const validateStep = (s: number) => {
    ...
    if (s === 1) {
      ...
      if (formData.paymentMethod === 'Boleto' && boletoFiles.length === 0) {
        errs.boletoFile = 'Selecione o anexo do boleto';
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedAuthorizer && selectedAccount && await submitRequest(finalFormData, currentProtocolId, selectedAuthorizer.id, selectedAccount.id, isUrgent)) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 md:gap-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="md:col-span-2">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <input
            type="date"
            className="flex-1 px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary/20 outline-none"
            value={getDateValue()}
          ...
          {showTimeInput ? (
            <div className="flex-1 flex gap-2">
              <input
                type="time"
                className="flex-1 px-4 py-3 rounded-xl border border-primary/30 bg-primary/5 focus:ring-2 focus:ring-primary/20 outline-none"
                value={getTimeValue()}
              ...
              <button
                onClick={() => {
                  ...
                  handleChange('dueDate', `${getDateValue()}T12:00`);
                }}
                className="p-3 text-slate-400 hover:text-danger"
              >
  ...
  const renderBoletoFields = () => (
    <div className="md:col-span-2 space-y-4">
      <label className="block text-sm font-medium text-slate-700">
        Anexos do Boleto <span className="text-accent">*</span>
      </label>
      ...
      {boletoFiles.map((file, idx) => (
        <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-white border border-slate-100">
          ...
          <button onClick={() => handleRemoveFile(idx, 'boleto')} className="p-1.5 text-slate-400 hover:text-danger">
            <X className="w-4 h-4" />
          </button>
        </div>
      ))}
      ...
      <div className="pt-2 space-y-2">
        ...
        {transferFiles.map((file, idx) => (
          <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-white border border-slate-100">
            ...
            <button onClick={() => handleRemoveFile(idx, 'transfer')} className="p-1.5 text-slate-400 hover:text-danger">
              <X className="w-4 h-4" />
            </button>
          </div>
        ))}
  ...
  const renderStep2 = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 md:gap-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-1.5">
          Verba Específica? <span className="text-accent">*</span>
        </label>
        <div className="flex gap-2">
          <button
            onClick={() => handleChange('isSpecificBudget', 'yes')}
            className={`flex-1 py-3 rounded-xl border ${formData.isSpecificBudget === 'yes' ? 'bg-primary/5 border-primary text-primary font-bold' : 'bg-white border-slate-200 text-slate-500'}`}
          >
            Sim
          </button>
          <button
            onClick={() => {
              handleChange('isSpecificBudget', 'no');
              handleChange('specificBudgetName', '');
            }}
            className={`flex-1 py-3 rounded-xl border ${formData.isSpecificBudget === 'no' ? 'bg-primary/5 border-primary text-primary font-bold' : 'bg-white border-slate-200 text-slate-500'}`}
          >
            Não
          </button>
        </div>
      ...
      {formData.paymentMethod === 'PIX' && (
        <div className="md:col-span-2 space-y-4">
          <label className="block text-sm font-medium text-slate-700">
            Tipo de chave PIX <span className="text-accent">*</span>
          </label>
          <div className="flex flex-wrap gap-2">
            {['cpf', 'cnpj', 'email', 'phone', 'random'].map(type => (
              <button
                key={type}
                onClick={() => handleChange('pixKeyType', type)}
                className={`px-4 py-2 rounded-xl text-xs font-black uppercase border transition-all ${formData.pixKeyType === type ? 'bg-primary text-white border-primary' : 'bg-white text-slate-500 border-slate-200'}`}
              >
                {type}
              </button>
            ))}
  ...
  const renderStep3 = () => (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-4 text-center md:text-left">
          Possui Nota Fiscal? <span className="text-accent">*</span>
        </label>
        <div className="grid grid-cols-2 gap-4">
          <button
            onClick={() => handleInvoiceOptionClick('yes')}
            className={`flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition-all ${formData.hasInvoice === 'yes' ? 'bg-primary/5 border-primary text-primary' : 'bg-white border-slate-100 text-slate-400'}`}
          >
            <FileText className="w-8 h-8" />
            <span className="font-bold">Sim, possuo</span>
          </button>
          <button
            onClick={() => handleInvoiceOptionClick('no')}
            className={`flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition-all ${formData.hasInvoice === 'no' ? 'bg-primary/5 border-primary text-primary' : 'bg-white border-slate-100 text-slate-400'}`}
          >
      ...
      {formData.hasInvoice === 'yes' && (
        <div className="space-y-4">
          <label className="block text-sm font-medium text-slate-700">
            Anexos da Nota Fiscal <span className="text-accent">*</span>
          </label>
          ...
          {invoiceFiles.map((file, idx) => (
            <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100">
              ...
              <button onClick={() => handleRemoveFile(idx, 'invoice')} className="p-1.5 text-slate-400 hover:text-danger">
                <X className="w-4 h-4" />
              </button>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-8 text-center">
            ...
            <p className="text-slate-600 text-sm mb-6">
              Ao prosseguir sem a NF, você se compromete a enviá-la imediatamente via WhatsApp para o financeiro.
            </p>
  ...
  const renderHeader = () => (
    <div className="bg-white border-b border-slate-100 py-3 px-6 flex justify-center fixed top-0 w-full z-50">
      <div className="text-[12px] font-medium text-slate-400 tracking-tight">
        CSP | <span className="text-slate-600">Central de Solicitação de Pagamento</span>
      </div>
    </div>
  );
      ...
      <div className="w-full bg-white rounded-[2.5rem] p-10 mb-10 shadow-2xl border border-slate-50 text-center relative overflow-hidden">
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <button
            ...
            className="p-3 bg-slate-100 rounded-2xl hover:bg-primary/10 transition-all"
          >
          ...
          <div className="bg-primary py-2 px-4 md:px-6 flex items-center shadow-lg h-12 md:h-14">
            <div className="flex items-center w-full justify-between gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-emerald-300 font-black text-[9px] md:text-xs uppercase tracking-[0.1em] md:tracking-[0.2em] hover:text-white transition-colors"
              >
                <Lock className="w-3.5 h-3.5 md:w-4 h-4" /> Administração
              </button>
              <button
                onClick={() => setIsInfoOpen(true)}
                className="flex items-center gap-1.5 text-white font-black text-[9px] md:text-[10px] uppercase tracking-[0.1em] md:tracking-[0.2em] md:w-40 justify-end hover:text-emerald-100 transition-colors"
              >
        ...
        {!isSuccess ? (
          <div className="w-full max-w-3xl animate-in fade-in zoom-in-95 duration-700">
            <div className="flex justify-between items-center mb-6">
              <button onClick={() => setView('welcome')} className="flex items-center gap-1.5 text-slate-400 hover:text-primary text-xs font-bold uppercase tracking-widest">
                <Home className="w-4 h-4" /> Início
              </button>
              <button onClick={resetForm} className="flex items-center gap-1.5 text-danger font-bold uppercase text-[10px]">
                <Trash2 className="w-4 h-4" /> Limpar tudo
              </button>
            </div>
            <div className="mb-4 md:mb-6 overflow-hidden">
              <span className="text-[9px] md:text-[10px] font-black text-primary uppercase tracking-[0.2em] md:tracking-[0.3em] mb-0.5 md:mb-1 block">
                Financeiro
              </span>
              <h1 className="text-2xl sm:text-3xl md:text-5xl font-bold text-slate-900 tracking-tighter mb-1 whitespace-nowrap overflow-hidden text-ellipsis">
                Nova Solicitação
              </h1>
            </div>
            ...
            <div className="flex justify-between items-center pt-4 border-t border-slate-200">
              <button
                onClick={prevStep}
                className={`flex items-center gap-2 text-slate-400 hover:text-slate-600 font-bold text-sm ${step === 0 ? 'invisible' : ''}`}
              >
                <ChevronLeft className="w-5 h-5" /> Voltar
              </button>
              {step < 4 ? (
                <Button onClick={nextStep} size="md">
                  Próximo <ChevronRight className="w-5 h-5 ml-1.5" />
                </Button>
              ) : (
